                    ...
                    >
                        <div class="question-header">
                            <span class="question-number"
                                >题目编号：{{ question.questionId }}</span
                            >
                            <span class="question-score"
                                >分值{{ question.score }}</span
                            >
                        </div>
                        <div
                            class="question-content"
                            v-html="formatText(question.questionContent)"
                        ></div>
                        <el-radio-group
            ...
            <template v-else>
                <div class="essay-question">
                    <div class="question-title">
                        ...
                        <span class="question-score"
                            >分值{{ questions[0]?.score }}</span
                        >
                    </div>
                    <div
                        v-for="(question, index) in questions[0]
                            ?.expoundingOptionInfos || []"
                        :key="index"
                    >
                        <p>
                    ...
                    <el-button type="primary" @click="submitEssayExam"
                        >提交申论答案</el-button
                    >
                </div>
            </template>
...
const fetchQuestions = async () => {
    try {
        ...
        const responseData = response?.data
            ? (response.data as unknown as Question[])
            : []
        ...
        if (subject.value === '行测') {
            ...
            responseData[0]?.expoundingOptionInfos
        ) {
            essayAnswers.value = new Array(
                responseData[0].expoundingOptionInfos.length
            ).fill('')
        }
    } catch (error) {
...
const submitExam = () => {
    ...
    totalScore.value = questions.value.reduce(
        (sum, q, i) =>
            answers.value[i] === q.correctAnswer ? sum + q.score : sum,
        0
    )
    //结果弹窗
...
const answerStatus = computed(() => {
    return questions.value.map((question, index) => {
        return answers.value[index] === question.correctAnswer
            ? 'correct'
            : 'incorrect'
    })
})
