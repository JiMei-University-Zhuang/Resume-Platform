  ...
  <div class="dashboard">
    ...
    <div class="additional-features">
      ...
      <el-row :gutter="24">
        <!-- AI聊天 -->
        <el-col :xs="24" :sm="12">
          <el-card class="additional-card" shadow="hover" @click="navigateTo('/chat')">
...
const updateSmartTips = async () => {
  ...
  smartTips.value = [
    predictedAction
      ? `根据您的使用习惯，推荐您现在${predictedAction}`
      : '欢迎使用AI职业规划平台！随着您的使用，我们会逐渐学习您的偏好，提供更精准的建议。',
...
// 在组件挂载后启动导览
onMounted(() => {
  const hasSeenIndexGuide = localStorage.getItem('hasSeenIndexGuide')
  if (!hasSeenIndexGuide) {
    nextTick(() => {
      setTimeout(() => {
        driverObj.drive()
...
.section-title::after {
  ...
  width: 80px;
  height: 3px;
  background: linear-gradient(90deg, #5ec89c, #67c23a);
...
.card-icon {
  ...
  margin-bottom: 20px;
  color: #5ec89c;
  background: rgba(94, 200, 156, 0.1);
  width: 70px;
  height: 70px;
  border-radius: 50%;
  display: flex;
