...
// 提交职业分析
export const submitCareerAnalysis = async (_data: CareerAnalysisForm): Promise<CareerAnalysisResult> => {
  // 实际项目中应该调用后端AI API
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
...
// 提交职业发展规划
export const submitCareerRoadmap = async (_data: CareerRoadmapForm): Promise<CareerRoadmapResult> => {
  // 实际项目中应该调用后端AI API
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
...
// 获取职业推荐
export const getCareerRecommendations = async (_data: CareerRecommendationForm): Promise<CareerRecommendationResult> => {
  // 实际项目中应该调用后端AI API
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        topRecommendations: [
          {
            title: '软件开发工程师',
            matchScore: 95,
            description: '负责设计、开发、测试和维护软件系统，使用编程语言和开发工具创建高质量的软件产品。',
          ...
          },
          {
            title: '数据科学家',
            matchScore: 89,
            description: '分析和解释复杂数据，使用统计和机器学习技术提取有价值的见解，支持决策制定。',
          ...
          },
          {
            title: '产品经理',
            matchScore: 82,
            description: '负责产品的战略规划、需求分析、功能设计和生命周期管理，协调各团队实现产品愿景。',
        ...
        industryTrends: [
          ...
          },
          {
            industry: '云计算',
            growthRate: '28%',
            description: '随着企业数字化转型加速，云计算技术人才的需求持续增长，特别是在云架构、安全和DevOps方面。'
          },
          {
            industry: '数据分析',
            growthRate: '25%',
            description: '大数据时代，数据分析师的重要性日益凸显，能够从数据中提取洞察的专业人才备受追捧。'
          }
        ],
        careerAdvice: '基于您的背景和兴趣，技术领域是最匹配的发展方向。建议深入学习软件开发或数据科学相关技能，同时关注人工智能等新兴领域的发展动态。'
      })
    }, 2200)
  })
} 