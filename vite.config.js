...
var copyLocalesPlugin = function () {
    return {
        ...
        closeBundle: function () { return __awaiter(void 0, void 0, void 0, function () {
            var srcDir, destDir, files, _i, files_1, file, srcFile, content, match, langObject, jsonFile, objStr, obj, enData, zhData;
                    ...
                    for (_i = 0, files_1 = files; _i < files_1.length; _i++) {
                        ...
                        if (file.endsWith('.ts')) {
                            ...
                            if (match && match[1]) {
                                ...
                                // 尝试将对象字符串转换为JSON
                                try {
                                    objStr = langObject.replace(/(\w+):/g, '"$1":')
                                        .replace(/,\s*}/g, '}')
                                        .replace(/'/g, '"');
                                    obj = JSON.parse(objStr);
                                    // 写入JSON文件
                                    fs.writeFileSync(jsonFile, JSON.stringify(obj, null, 2), 'utf-8');
                                    console.log("\u2705 \u6210\u529F\u5C06 ".concat(file, " \u8F6C\u6362\u5E76\u590D\u5236\u5230 ").concat(jsonFile));
                                }
                                catch (err) {
                                    ...
                                    try {
                                        if (file === 'en.ts') {
                                            ...
                                            fs.writeFileSync(path.join(destDir, 'en.json'), JSON.stringify(enData, null, 2), 'utf-8');
                                            console.log("\u2705 \u4F7F\u7528\u5907\u7528\u65B9\u6848\u6210\u529F\u5C06 en.ts \u8F6C\u6362\u5E76\u590D\u5236");
                                        }
                                        else if (file === 'zh-cn.ts') {
                                            zhData = require('./src/locales/zh-cn').default;
                                            fs.writeFileSync(path.join(destDir, 'zh-cn.json'), JSON.stringify(zhData, null, 2), 'utf-8');
                                        ...
                                        // 最终备用方案：手动复制已知内容
                                        if (file === 'en.ts') {
                                            fs.copyFileSync(path.resolve(__dirname, 'src/locales/en.json'), path.join(destDir, 'en.json'));
                                        }
                                        else if (file === 'zh-cn.ts') {
                                            fs.copyFileSync(path.resolve(__dirname, 'src/locales/zh-cn.json'), path.join(destDir, 'zh-cn.json'));
                                        }
                                    }
                                }
                            }
